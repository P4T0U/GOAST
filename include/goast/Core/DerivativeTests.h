...
public:
  ...
  void plotSingleDirection ( const VectorType & testPoint, const VectorType & testDirection, const std::string saveName ) const {
  ...
  RealType computeDiffQuotient ( const VectorType & testPoint, const VectorType & testDirection, RealType tau, bool centralDiff = false ) const {

    VectorType shiftedPoint = centralDiff ? (testPoint - tau/2. * testDirection) : testPoint; // - _stepSize/2. * testDirection;
  ...
  void testSingleDirection ( const VectorType & testPoint, const VectorType & testDirection, RealType tau = -1. ) const {       
    ...
    std::cerr << std::abs( (computeDiffQuotient( testPoint, testDirection, stepSize ) - gateaux) / gateaux ) << std::endl;
  }
  ...
  void testRandomDirections( const VectorType & testPoint, unsigned numOfRandomDir, const bool detailedOutput = false, RealType tau = -1. ){
...
public:
  ...
  : _F ( F ),
   ...
   //
   void plotRandomDirections ( const VectorType & testPoint, int numOfRandomDir, const std::string saveNameStem, const bool testNonNegEntriesOfHessianOnly = true ) const {
  ...
  //
  void plotSingleDirection ( const VectorType & testPoint, const VectorType & innerDirection, const VectorType & outerDirection, const std::string saveName ) const {
  ...
  //
  void plotSingleDirection ( const VectorType & testPoint, int innerDirection, int outerDirection, const std::string saveName ) const {
  ...
  //
  RealType computeDiffQuotient ( const VectorType & testPoint, const VectorType & outerDir, const VectorType & innerDir, RealType tau ) const {
  ...
  //
  void testSingleDirection( const VectorType & testPoint, const VectorType & outerDir, const VectorType & innerDir, RealType tau = -1. ) const {
    ...
    std::cerr <<  std::abs( (computeDiffQuotient ( testPoint, outerDir, innerDir, stepSize ) - gateaux ) / gateaux )  << std::endl;        

  }
  
  //
  void testAllDirections( const VectorType & testPoint, RealType tau = -1. ) const {
    ...
    for( int i = 0; i < dimRange; i++ ){ 
      ...
      for( int j = 0; j < numDofs; j++ ){
        ...
        innerDir[j] = 1.;
        RealType gateaux = gradientWRTOuterDirection.dot( innerDir );
        error[j] =  std::abs( (computeDiffQuotient ( testPoint, outerDir, innerDir, stepSize ) - gateaux) / gateaux );        
      }      
      printVector( error, 10 );
    }
  }
  ...
  //
  void testRandomDirections( const VectorType & testPoint, unsigned numOfRandomDir, RealType tau = -1., bool testNonNegEntriesOfHessianOnly = true ){
      ...
      for( unsigned i = 0; i < numOfRandomDir; ++i ){
        outerDir.setZero();   
        innerDir.setZero();
        outerDir[ std::rand()%dimRange ] = 1.;
        innerDir[ std::rand()%numDofs ] = 1.;
        VectorType gradientWRTOuterDirection( numDofs );        
        gradientWRTOuterDirection  = JacobiMatrixTrans * outerDir;
        RealType gateaux = gradientWRTOuterDirection.dot( innerDir );
...
public:
  ...
  //! TODO
  void testSingleDirection( const VectorType & testPoint, const VectorType & dir1, const VectorType & dir2, const VectorType & dir3, RealType tau = -1. ) const {
