...
public:
  ...
  inline bool less( VertexHandle vh0, VertexHandle vh1 ) { return _prio[vh0.idx()] < _prio[vh1.idx()]; }

  inline bool greater( VertexHandle vh0, VertexHandle vh1 ) { return _prio[vh0.idx()] > _prio[vh1.idx()]; }

  inline int get_heap_position( VertexHandle vh ) { return _pos[vh.idx()]; }

  inline void set_heap_position( VertexHandle vh, int pos ) { _pos[vh.idx()] = pos; }
...
public:
  SimultaneousRandomMetric( std::vector<TriMesh> &opMeshes ) : SimultaneousErrorMetricBase<ConfiguratorType>(
          opMeshes ), _numOfMeshes( opMeshes.size()) { std::srand( std::time( 0 )); }
...
public:
  ...
  SimultaneousQuadricErrorMetric( std::vector<TriMesh> &opMeshes, RealType edgeLengthParam,
                                  bool computeOptimalPositions, bool extendedPostProceesing )
          : SimultaneousErrorMetricBase<ConfiguratorType>( opMeshes ),
            _numOfMeshes( opMeshes.size()),
            _initialized( false ),
...
#pragma omp parallel for
  ...
  void postprocessCollapse( const CollapseInfo &Collapse, std::vector<VertexHandle> &support ) {
    ...
    for ( int i = 0; i < _numOfMeshes; i++ )
      this->_opMeshes[i].property( _quadrics[i], Collapse.v1 ) += this->_opMeshes[i].property( _quadrics[i],
                                                                                               Collapse.v0 );
  }
...
protected:
  ...
  void computeOptimalPositions( TriMesh &opMesh,
    ...
    for ( typename TriMesh::EdgeIter e_it = opMesh.edges_begin(); e_it != v_end; ++e_it ) {
      ...
      VecType v_bar;
      if (( std::abs( detA ) > std::numeric_limits<RealType>::epsilon()) && !opMesh.is_boundary( vh0 ) &&
          !opMesh.is_boundary( vh1 )) {
  ...
  //
  void updateFaceMatrix( TriMesh &opMesh, OpenMesh::VPropHandleT<QuadricType> &quadric, const FaceHandle &fh,
...
// returns dihedral angle between n1 and n2 at an edge e
  RealType getDihedralAngle( const Point &nk, const Point &nl, const Point &e ) const {
    ...
    return dotProduct( crossprod, e ) < 0. ? -std::acos( min( dotProduct( nk, nl ), 1. )) : std::acos(
            min( dotProduct( nk, nl ), 1. ));
  }
...
public:
  ...
  SimultaneousDecimater( const std::vector<TriMesh> &meshes, bool Quiet = false )
          : _numOfMeshes( meshes.size()), _numOfOriginalVertices( meshes[0].n_vertices()), _opMeshes( meshes ),
            _quadric( _opMeshes ), //_heap(NULL),
            _prio( _numOfOriginalVertices ), _pos( _numOfOriginalVertices ), _collapseTarget( _numOfOriginalVertices ),
            _fixedVertices( _numOfOriginalVertices ), _quiet( Quiet ), _updatePositions( false ) {
    _quadric.initialize();
  }
  ...
  int flipEdgesWithBadAngles( RealType angleThreshold = 3.0 ) {
    ...
    for ( int idx = 0; idx < _numOfMeshes; idx++ ) {
      // run over all edges
      for ( typename TriMesh::EdgeIter e_it = _opMeshes[idx].edges_begin();
            e_it != _opMeshes[idx].edges_end(); ++e_it ) {
  ...
  static void computeDecimatedBoundaryMask( const DecimationInfo<ConfiguratorType> &decInfo, const BitVector &fineMask,
                                            BitVector &coarseMask ) {
...
#pragma omp parallel for
  ...
  *
  **/
  bool isSameOrientation( const TriMesh &mesh, HalfedgeHandle edge, HalfedgeHandle curr_to_v0, VertexHandle curr,
